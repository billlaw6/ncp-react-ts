...
import { StoreStateI, VasTypeI, FlowStatusI } from "../../constants/interface";
...
class CaseRecordForm extends React.Component<CaseRecordFormProps & CaseRecordPropsI, CaseRecordStateI> {
  constructor(props: CaseRecordFormProps & CaseRecordPropsI) {
    super(props);
    this.state = {
      // 本页面要用的字典
      vasTypeList: [],
  ...
  handleDownloadClick = () => {
    ...
    axios({
      method: "get",
      url: downloadUrl,
      params: this.props.caseRecordSearchForm,
      headers: {
  ...
  render() {
    ...
    const { vasTypeList, flowStatusList } = this.state; // 析构出来避免后面取值时this指代变化问题
          ...
          <Form className="case-record-form" name="case-record" onSubmit={this.handleSubmit}>
            ...
            <Item label="桥-OM3_靶血管术后狭窄程度%" colon={false}>
              {getFieldDecorator("B_OM3_target_vas_narrow_ratio_after", {
                rules: [{ required: true, message: "请录入桥-OM3_靶血管术后狭窄程度%" }],
...
export default connect(mapStateToProps, mapDispatchToProps)(WrappedCaseRecordForm);
